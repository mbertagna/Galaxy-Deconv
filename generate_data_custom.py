...
def generate_data_deconv(data_path, n_train=40000, n_test=10000, load_info=True,
    ...
    Args:
        ...
        snr (int or list, optional): The list of SNR to be simulated for testing. Defaults to `100`.
    ...
    for k in tqdm(range(0, n_train + n_test)):
        ...
        if k >= n_train:
            ...
            if isinstance(snr, list):
                for s in snr:
                    alpha = s * sigma / torch.sqrt((gal_image_down**2).sum()) # Scale the flux of galaxy to meet SNR
                    ...
                    # Save Images.
                    if not os.path.exists(os.path.join(data_path, f'gt_{s}')):
                        os.mkdir(os.path.join(data_path, f'gt_{s}'))
                    if not os.path.exists(os.path.join(data_path, f'obs_{s}')):
                        os.mkdir(os.path.join(data_path, f'obs_{s}'))
                    torch.save(gt_snr.clone(), os.path.join(data_path, f'gt_{s}', f"gt_{k}.pth"))
                    torch.save(obs_snr.clone(), os.path.join(data_path, f'obs_{s}', f"obs_{k}.pth"))
